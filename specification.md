...
This MCP takes the following information of any codebase:
    ...
    3. `git diff HEAD~3` - After

From these information, the tool generates a hash from it and put everything in a tuple: 1. State Tuple: <STATE_NUMBER, USER_PROMPT, BRANCH_NAME, GIT_DIFF_INFO, HASH>

Transitions are also stored in a tuple: 2. Transition Tuple: <ID/INDEX, CURRENT_STATE, NEXT_STATE, DATE_TIME>

## Tools Definition

**genesis()** - Initialize a state machine for the managed project and creates the state #0 (zero). Creates a branch called 'codebase-state-machine' copied from the current branch where the tool was called from. The new branch will be stored in a local dedicated volume in the docker container. It MUST be the first tool to be called in the MCP server initialization

**new_state_transition(string prompt, number previous_state)** - Performs a transition from the user prompt/request, at the current state (which will be the previous_state), to a newly created state. The transition is recorded. Generally, is performed **AFTER** the AI Agent completes a task for a given user prompt

**arbitrary_state_transition(number current_state, number next_state)** - Performs an arbitrary state transition from the current state to a given state number. The transition is recorded

**get_current_state_number()** - Returns only the number of the current managed state

**get_current_state_info()** - Returns information (tuple) of the current managed state. Provides the context needed for the AI Agent and should be called **BEFORE** submitting any prompt/request for it
